    ...
    return (
        <div style={styles.container}>
            <div style={styles.topBar}>
                <div style={styles.leftTopBar}>
                    ...
                    <div style={styles.logoContainer}>
                        <img src="/images/logo-orizon.jpeg" alt="Orizon Cottage" style={styles.logo} onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; }} />
                    </div>
                    <div><h1 style={styles.title}>Orizon Cottage</h1><p style={styles.subtitle}>Gestión de Convivencia</p></div>
                </div>
                <div style={styles.userArea}>
                    <div style={styles.schoolLogoContainer}>
                        <img src="/images/logo-cabana.png" alt="I.E. La Cabaña" style={styles.schoolLogo} onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; }} />
                    </div>
            ...
            <div style={styles.mainContainer}>
                <div style={{ ...styles.sidebar, ...(isMobile && { position: "fixed", left: 0, top: 70, transform: menuOpen ? "translateX(0)" : "translateX(-100%)", width: "80%", maxWidth: "300px", zIndex: 100 }) }}>
                    <h3 style={styles.sidebarTitle}>Panel Principal</h3>
                    <ul style={styles.menuList}>
                        {menuItems.map((item) => (
                            <li key={item.id} style={{ ...styles.menuItem, ...(activeSection === item.id && styles.menuItemActive) }} onClick={() => { setActiveSection(item.id); if (isMobile) setMenuOpen(false); }}>
                                <span style={styles.menuIcon}>{item.icon}</span>{item.label}
                            </li>
                        ))}
                    </ul>
                </div>
...
const styles = {
    ...
    logoContainer: { width: "40px", height: "40px", display: "flex", alignItems: "center", justifyContent: "center" },
    logo: { maxWidth: "40px", maxHeight: "40px", objectFit: "contain" },
    title: { margin: 0, fontSize: "18px" },
    subtitle: { margin: 0, fontSize: "10px", opacity: 0.9 },
    userArea: { display: "flex", alignItems: "center", gap: "12px" },
    schoolLogoContainer: { width: "35px", height: "35px", display: "flex", alignItems: "center", justifyContent: "center" },
    schoolLogo: { maxWidth: "35px", maxHeight: "35px", objectFit: "contain" },
